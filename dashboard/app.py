...
from src.preprocessing.pipeline import PreprocessorConfig
...
def _render_result_card(result: dict, image: Image.Image) -> None:
    ...
    with col2:
        ...
        st.caption(f"Threshold: {result['threshold']:.2f}  |  Latency: {result['inference_ms']:.1f} ms")
...
tab_verify, tab_demo, tab_stats = st.tabs(
    ["Document Verifier", "Demo Mode", "Session Stats"]
)
...
with tab_demo:
    ...
    st.caption(
        "Run inference on the pre-generated synthetic samples in `data/samples/`."
    )
