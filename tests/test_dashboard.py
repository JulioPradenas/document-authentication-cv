...
class TestLabelColor:
    def test_authentic_returns_green(self):
        from dashboard.app import _etiqueta_color

        assert _etiqueta_color("authentic") == "🟢"

    def test_forged_returns_red(self):
        from dashboard.app import _etiqueta_color

        assert _etiqueta_color("forged") == "🔴"
...
class TestAppSmoke:
    ...
    def test_sidebar_has_threshold_slider(self, mock_predictor):
        ...
        with patch("dashboard.app.load_predictor", return_value=mock_predictor):
            ...
            assert any("umbral" in lbl.lower() for lbl in slider_labels)
    ...
    def test_three_tabs_present(self, mock_predictor):
        ...
        with patch("dashboard.app.load_predictor", return_value=mock_predictor):
            ...
            expected = {
                "Sube un documento para verificar",
                "Demo — muestras sintéticas",
                "Estadísticas de sesión",
            }
            assert expected <= set(headers), f"Pestañas no encontradas, got: {headers}"
